...
class marker(object):
    ...
    def mark(self,loc,content):
        if loc in self._db:
            self._db[loc]=cPickle.dumps(cPickle.loads(self._db[loc])+content)
        else:
            self._db[loc]=cPickle.dumps(self._default+content)
        self.sync()
    ...
    def moveto(self,loc):
        if loc not in self._db:
            self._db[loc]=cPickle.dumps(self._default)
        self._db.set_location(loc)
        return cPickle.loads(self._db[loc])

    def report(self,st,en):
        #returns the content from $current_loc - noffset$ to, and including
        # $current_loc+poffset$.
        return [(cPickle.loads(self._db[loc]) if loc in self._db else self._default) for loc in xrange(st,en+1)]
