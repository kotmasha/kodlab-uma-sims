...
def start_experiment(run_params):
    ...
    TRIAL_CYCLES = int(run_params['trial_cycles'])
    TRIAL_PERIODS = int(run_params['trial_periods'])
    TOTAL_CYCLES = TRAINING_CYCLES+TRIAL_PERIODS*TRIAL_CYCLES
    ...
    # initial number of cheeses is 1 (training)
    # number of cheeses for each trial:
    cheeseNum=int(run_params['number_of_cheeses'])
    cheeseParams={
        'nibbles':TOTAL_CYCLES,
        'nibbleDist':1,
        }
    arena.addRandomCheeses(1,cheeseParams)

    # add a mouse at the position of one of the cheeses (training)
    mouseAttr={'viewSize':viewportSize,'direction':DIRS[rnd(4)]}
    ...
    # a single state update to fill the state deques
    ...
    # trial periods
    for ind in xrange(TRIAL_PERIODS):
        # move the cheese to a random location:
        chl=arena.getCheeses()
        chk=chl.keys()
        for ch in chk:
            chl[ch].remove()
        ...
        mouse.teleport()
        # udpate the arena
        EX.update_state([cid_lt,cid_rt,cid_fd,cid_bk])
        # commence trial
        for t in xrange(TRIAL_CYCLES):
            EX.update_state()
            recorder.record()
