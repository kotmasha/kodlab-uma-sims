...
def start_experiment(run_params):
    ...
    # starting position for mouse in the training arena
    mouseAttr={'viewSize':viewportSize,'direction':[North,West,South,East][rnd(4)]}    
    mouseInitPos=training_arena._objects['ch1']._pos
    training_arena.addMouse('mus',mouseInitPos,mouseAttr,{})
    
    ## Working arena
    #
    arena = Arena_base(XBOUNDS,YBOUNDS,eval(run_params['out_of_bounds']))
    # initial number of cheeses is set to satisfy, on average, the
    # cheesesPerView constraint
    ...
    # add a mouse at a random position
    ...
    #pos before direction to pos uses state[direction][0] which is the direction of the last state
    ...
    def arena_update(state):
        ...
        ar=training_arena if state[id_count][0]<=BURN_IN_CYCLES else arena
        state[id_arena][0].update_objs(command)
        return ar
    INIT=training_arena
    EX.construct_measurable(id_arena, arena_update,[INIT],depth=0)
