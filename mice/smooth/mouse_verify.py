...
x0,x1=eval(preamble['xbounds'])
dx=x1-x0
y0,y1=eval(preamble['ybounds'])
dy=y1-y0
fig.suptitle('Mice: mean and std. deviations of distance to nearest cheese as a function of time\n after '+str(preamble['training_cycles'])+'cycles of training by random exploration in a '+str(dx)+'*'+str(dy)+' environment.',fontsize=22)
...
ymin,ymax=plt.ylim()
plt.plot([preamble['training_cycles'],preamble['training_cycles']],[ymin,ymax],'-bo',label='training period ends')
plt.plot(preamble['training_cycles'],ymin,preamble['training_cycles'],ymax,'bo',)
ax.legend()
