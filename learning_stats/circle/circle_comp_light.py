...
def start_experiment(run_params):
    ...
    def xsensor(state,m,width):
        return dist(state[id_pos][0],m)<=width
    ...
    for ind in xrange(X_BOUND):
        ...
        EX.construct_sensor(id_tmp, partial(xsensor,m=ind,width=BEACON_WIDTH))  # constructs the measurables associated with the sensor
        for typ in ORDERED_TYPES:
