...
TARGET_GROUND=np.array([[pos==0 for pos in xrange(ENV_LENGTH)] for ind in xrange(DURATION)]).T
...
#main plot
ax_dist.plot(t+0.5,dist,'-k',alpha=1,label='Run \#'+str(NUM)+' of '+str(preamble['Nruns'])+': distance to target')
#end of training
ax_dist.plot([preamble['burn_in_cycles']+2.5,preamble['burn_in_cycles']+2.5],[-1,ENV_LENGTH/2+1],'-b.',label='training period ends')
ax_dist.legend()
...
for key,ax in zip(ACCESS_KEYS,axes[1:]):
    ...
    ax.imshow(PREDICTION_TRAJECTORY[key], origin='lower', cmap=plt.cm.Greens, vmin = 0, vmax = 1, aspect='auto',interpolation='none',alpha=.7,extent=EXTENT,label='predicted position')
    ...
    ax.plot(t+0.5,np.array(POS),'.r',alpha=1,label='agent\'s position')
    #end of training
    ax.plot([preamble['burn_in_cycles']+2.5,preamble['burn_in_cycles']+2.5],[-1,ENV_LENGTH],'-b.',label='training period ends')
    ax.plot(t+0.5,DIVERGENCES[key]-DIVERGENCES[nc(key)],'-g',alpha=1,label='divergence difference')
    if key==ACCESS_KEYS[-1]:
