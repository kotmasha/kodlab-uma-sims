...
DATA_BOT=filter(lambda item: item[BURN_IN]<=RADIUS,DATA['dist'])
...
#for datum in DATA_BOT:
#    plt.plot(t,datum,'^r',alpha=1,label='single sample')

# mean over training walks ending closer to antipode
DATA_TOP=filter(lambda item: item[BURN_IN]>RADIUS,DATA['dist'])
...
#for datum in DATA_TOP:
#    plt.plot(t,datum,'ob',alpha=1,label='single sample')


ymin,ymax=plt.ylim()
plt.plot([preamble['burn_in_cycles']+2,preamble['burn_in_cycles']+2],[ymin,ymax],'--b.',label='training period ends')
ax.legend()
