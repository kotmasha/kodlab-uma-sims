...
RADIUS=ENV_LENGTH/4
MY_FILTER=lambda ind: DATA['dist'][ind][BURN_IN]<=RADIUS
notMY_FILTER= lambda ind: not MY_FILTER(ind)
#according to prediction quality
#MY_FILTER=lambda ind: all([sum(DATA['pred'+key][ind][-1])>0 for key in ACCESS_KEYS])
...
DATA_BOT=[DATA['dist'][ind] for ind in filter(MY_FILTER,xrange(NRUNS))]
#for key in ACCESS_KEYS:
#    print DATA['pred'+key][0][-1]
#print MY_FILTER(0)
#for key in ACCESS_KEYS:
#    print DATA['pred'+key][5][-1]
#print MY_FILTER(5)
#exit(0)
...
DATA_TOP=[DATA['dist'][ind] for ind in filter(notMY_FILTER,xrange(NRUNS))]
