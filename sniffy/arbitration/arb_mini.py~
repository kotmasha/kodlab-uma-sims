...
def start_experiment(run_params):
    ...
    # triggers for arbiter's sensors    
    ARB_WITH_GEO=True #toggle geoegraphic sensors
    ARB_WITH_FAILURE=False #toggle failure sensors
    ...
    def action_RT(state):
        ...
        if arb_decided: #If arbiter is active...
            if state[id_arb_coin][0]: #...and the coin-flip went "heads"...
                ...
                return rt_decided #...keep the original decision by RT.
    ...
    def action_LT(state):
        ...
        if arb_decided: #If arbiter is active...
            if state[id_arb_coin][0]: #...and the coin-flip went "heads"...
                return lt_decided #...keep the original decision by LT.
            else: #If the coin-flip went "tails"...
    ...
    #     ARB  :  #toF < F* (failure prevented by ARB)
    #     ARB* :  #toF < F  (failure enforced by ARB*)
    ...
    ACCESS={}
    for agent_id in [id_lt,id_rt,id_arb]:
    ...
    #
    ### Access to internal data for each agent
    #
    ...
    # get weight data from arbiter
    id_imps={}
    
    def get_imps(state,agent_id,snap_ind):
        return convert_full_implications(
            state[id_acc[(agent_id,snap_ind)]][0]['npdirs']
        ).tolist()
    INITSIZE={id_arb:4*X_BOUND*ARB_WITH_GEO+8*ARB_WITH_FAILURE,
              id_lt:4*X_BOUND,
              id_rt:4*X_BOUND,
              }
    for aid in [id_arb,id_lt,id_rt]:
        for tok in ['plus','minus']:
            id_imps[(aid,tok)]=EX.register('imps_'+aid+('+' if tok=='plus' else '-'))
            EX.construct_measurable(id_imps[(aid,tok)],partial(get_imps,agent_id=aid,snap_ind=tok),[np.identity(INITSIZE[aid])],depth=0)
    ...
    # INTRODUCE DELAYED FAILURE SENSORS TO ARBITER:
    if ARB_WITH_FAILURE:
        for token in ['plus','minus']:
            delay_sigs=[]
            #identify the failure sensors:
            delay_sigs.extend([ARB.generate_signal([sid],token) for sid in [id_toF,id_F]])
            #construct the corresponding delayed sensors in each snapshot:
            ARB.delay(delay_sigs,token)

    # INTRODUCE DELAYED BEACON SENSORS TO ALL AGENTS:
    GEO_AGENTS=[RT, LT, ARB] if ARB_WITH_GEO else [RT,LT]
    for agent in GEO_AGENTS:
        for token in ['plus', 'minus']:
            delay_sigs = []
            #identify the beacon sensors:
    ...
    recorder.addendum('rt_LabelsPlus',RT.make_sensor_labels('plus'))
    recorder.addendum('rt_LabelsMinus',RT.make_sensor_labels('minus'))
    recorder.addendum('lt_LabelsPlus',LT.make_sensor_labels('plus'))
    recorder.addendum('lt_LabelsMinus',LT.make_sensor_labels('minus'))
    recorder.addendum('arb_LabelsPlus',ARB.make_sensor_labels('plus'))
    recorder.addendum('arb_LabelsMinus',ARB.make_sensor_labels('minus'))
